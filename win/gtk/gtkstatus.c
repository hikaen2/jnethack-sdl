...
#ifdef JNETHACK
    {
	{NOVALUE, STAT_TYPE_CHAR_P, NULL,	"鎧",   	(void *)&u.uac},
	{NOVALUE, STAT_TYPE_INT_P, NULL,	"金",  		(void *)&u.ugold},
	{NOVALUE, STAT_TYPE_F_INT, NULL,	"レベル",	stat_lvl},
	{NOVALUE, STAT_TYPE_F_CHAR_P, NULL,	"属性",		stat_align},
	{NOVALUE, STAT_TYPE_INT_P, f_time,	"歩",		(void *)&moves},
#ifdef EXP_ON_BOTL
	{NOVALUE, STAT_TYPE_INT_P, f_exp,	"経験", 	(void *)&u.uexp},
#else
	{NOVALUE, STAT_TYPE_NULL},
#endif
#ifdef SCORE_ON_BOTL
	{NOVALUE, STAT_TYPE_F_INT, f_score,	"スコア",	stat_score},
#else
	{NOVALUE, STAT_TYPE_NULL},
#endif
    },
    {
	{NOVALUE, STAT_TYPE_F_INT, NULL,	"強さ",		stat_stat, (void *)A_STR},
	{NOVALUE, STAT_TYPE_F_INT, NULL,	"素早さ",	stat_stat, (void *)A_DEX},
	{NOVALUE, STAT_TYPE_F_INT, NULL,	"耐久力",	stat_stat, (void *)A_CON},
	{NOVALUE, STAT_TYPE_F_INT, NULL,	"知力",		stat_stat, (void *)A_CON},
	{NOVALUE, STAT_TYPE_F_INT, NULL,	"賢さ",		stat_stat, (void *)A_WIS},
	{NOVALUE, STAT_TYPE_F_INT, NULL,	"魅力",		stat_stat, (void *)A_CHA},
    },
#else	/* JNETHACK */
    {
...
#ifdef JNETHACK
    if(Blind)
	gtk_label_set_text(GTK_LABEL(blin), "盲目");
    else
	gtk_label_set_text(GTK_LABEL(blin), "");

    if(Confusion)
	gtk_label_set_text(GTK_LABEL(conf), "混乱");
    else
	gtk_label_set_text(GTK_LABEL(conf), "");

    if(Sick){
	if(u.usick_type & SICK_VOMITABLE)
	    gtk_label_set_text(GTK_LABEL(sick), "食中毒");
	else
	    gtk_label_set_text(GTK_LABEL(sick), "病気");
    }
    else
	gtk_label_set_text(GTK_LABEL(sick), "");

    if(Stunned)
	gtk_label_set_text(GTK_LABEL(stun), "眩暈");
    else
	gtk_label_set_text(GTK_LABEL(stun), "");

    if(Hallucination)
	gtk_label_set_text(GTK_LABEL(hall), "幻覚");
    else
