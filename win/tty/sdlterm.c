...
/*
 * wintty.c and topl.c read CO, LI, nh_CM and ul_hack out of these.  nh_CM
 * must be non-null or tty_curs() takes the "terminal has no cursor
...
#ifdef WIN32
/*
 * win/tty/wintty.c defines g_putch() only "#ifndef WIN32"; on Windows it
 * expects the port to bring its own, the way sys/winnt/nttty.c does for
 * the console.  This build does not compile nttty.c -- everything else it
 * would supply is here already -- so the copy lives here.
 *
 * It is the ASCIIGRAPH && !NO_TERMS branch of wintty.c's version, minus
 * the PC9800 case, so the two builds route the same byte to the same
 * place.  That equivalence is the point: the Windows binary is verified
 * by diffing its screens against the Unix ones (test/wincompare.sh).
 */
void
g_putch(in_ch)
int in_ch;
{
    extern boolean GFlag, HE_resets_AS;		/* win/tty/wintty.c */
