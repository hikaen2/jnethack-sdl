...
| 8 | **SDL が半角カナを2セル幅で描く** | `sdl_puteuc()` が `sdl_put_wide(..., 2)` と幅を固定。SS2 の半角カナは2バイトだが1桁で、tty 側は1桁に描くので**両バックエンドが食い違う**。`b1 == 0x8E` の判定2行で直る |

1〜3 と 8 は `topten.c` / `sdlterm.c` の局所修正で、`mbchar` などの基盤なしに移植できる。
