...
進捗: **Phase 0〜5 完了**。内部コードは UTF-8、追跡下の全テキストファイルが UTF-8。
テストは10スイート。残る判断事項は §10、`experimental/sdl-jnethack-1.1.5` への
逆移植候補は §11 を参照。
...
移行そのものは完了しているが、以下は「どちらが正しいか」がコードの外にあるため、
判断を保留して現状のままにしてある。

### 10.1 `regularize()` の11バイト制限

`sys/unix/unixunix.c` の `regularize()` は System V の14文字ファイル名制限のために
セーブファイル名を11バイトに切る。文字境界で切るようには直したが、制限そのものは
そのまま。11バイトのうち4バイトは uid が使うので、プレイヤ名に残るのは漢字2文字強
——EUC-JP 時代の3文字強より減る。似た名前の2人がセーブファイルを共有しやすくなる。

`include/unixconf.h:28` の `SYSV` 定義に連動しており、現代の Linux では ext4 が
255バイトを許すのでこの制限に実体はない。ただし `SYSV` は他の挙動も切り替えるため、
外すかどうかは「このツリーがどのシステムを対象とするか」の判断になる。

### 10.2 `engrave.c` の `maxelen`

刻文の長さ上限は杖の充填数由来のバイト予算で、`strlen()` と比較されており自己整合
している。着地点は文字境界に強制したが、単位はバイトのまま。漢字1文字が2充填分か
3充填分かはゲームバランスの問題で、符号化の問題ではない。

### 10.3 `SOKUON` / `HATSUON` の名前

`japanese/jconj.c` の2つの定数は実際の出力と入れ替わっている（`SOKUON` が「ん」を、
`HATSUON` が「っ」を書く）。出力は正しい。改名は64語の分類を1つ間違えるだけで
静かに壊れるため、コメントで記録するにとどめた。

### 10.4 `default` 分岐の `cell <= 84`

`jrndm_replace` の既定分岐は `cell <= 84` を検査するが、他の全ての検査の形と、
16〜84区が漢字であることからすると `row <= 84` の意図に見える。定義された挙動を
持つ生きた条件であり、変えるとどの文字が劣化するかが変わるため、そのままにした。

## 11. `experimental/sdl-jnethack-1.1.5` へ逆移植すべきもの

このブランチで直したもののうち、**符号化と無関係な欠陥**は EUC-JP のままの
`experimental/sdl-jnethack-1.1.5` にも存在する。下記は sdl ブランチの実コードに
当たって未修正であることを確認済み。判断は保留、作業は未着手。

### 11.1 移植すべき（8件）

| | 内容 | sdl ブランチでの根拠 |
|---|---|---|
| 1 | **topten: 偽の魔除けでの脱出が「死亡」として記録される** | `strncmp(jdeath, "偽物の魔除けを掴まされ", 24)` のリテラルは22バイト。`strncmp` がリテラルの終端に達して必ず不一致になり、分岐が一度も成立しない |
| 2 | **topten: death フィールドのバッファ溢れ** | `strncat` が DTHSZ(60) まで埋めた後、`Strcat`（境界検査のない `strcat`）が最大12バイト追記。バッファは61バイト。`killer` はモンスター名で、プレイヤーが名付けたモンスターは `PL_NSIZ` バイトの名前を持つため到達可能 |
| 3 | **topten: `readentry()` の `fscanf` に幅指定がない** | `%[^,],%[^\n]` が `name[NAMSZ+1]` と `death[DTHSZ+1]` へ無制限に書き込む |
| 4 | **`split_japanese` が漢字を2バイトの間で割る** | 探索窓の下限がバイト数で文字境界に整列していない。EUC ビルドで実証済み（`xxx…漢字…xxx` を pos=47 で分割すると「字」が割れる）。壊れた半分が画面と `record` ファイルに出る |
| 5 | **`regularize()` がセーブファイル名を漢字の途中で切る** | 11バイト切りで uid が4バイトを使うため、4文字以上の名前は7バイト目＝奇数位置で切れる |
| 6 | **`do_name.c` が未初期化の `buf` を読む** | `is_kanji2(buf, lth-1)` が `strncpy` で `buf` を埋める前に実行され、結果も次行の `lth = PL_PSIZ` で上書きされる。完全な空振り |
| 7 | **`jrndm_replace` が未初期化の `cc[2]` を読む** | `unsigned char cc[3]` の [0],[1] しか設定されない。全角小文字の劣化がスタックの残骸次第になる |
| 8 | ~~**SDL が半角カナを2セル幅で描く**~~ **移植済み** | `sdl_puteuc()` が `sdl_put_wide(..., 2)` と幅を固定していた。SS2 の半角カナは2バイトだが1桁で、tty 側は1桁に描くので**両バックエンドが食い違っていた**。EUC ブランチが `593647e` で `b1 == 0x8E` の判定を入れ、このブランチはその上に載っている。こちら側では `sdl_puteuc()` 自体が不要になったので削除した |

1〜3 は `topten.c` の局所修正で、`mbchar` などの基盤なしに移植できる。
4〜7 も同様。ただし元のコミットは UTF-8 化と混ざっているため、cherry-pick ではなく
EUC 向けに書き直す形になる。

### 11.2 移植すべきでない

いずれも EUC-JP では**現状が正しい**。

- 桁とバイトを分離した表示修正すべて（`compress_str`、`tty_putstr` の `NHW_BASE` と
  `NHW_TEXT`、`process_text_window`、`process_menu_window`、`topl.c`、`rip.c`、
  `NHW_STATUS` の差分ループ）——漢字が2バイト＝2桁なので一致している
- `makedefs` の `p+=2` と80バイト読み——日本語が必ず偶数バイト、`quest.txt` の
  最長行も63バイトなので当たらない
- `questpgr.c` のバッファ拡大——`cvt_buf[64]` が溢れるのは `PL_NSIZ` を128に
  したためで、EUC の32なら足りている
- テストハーネスの charset 変更（`compare.sh`、`wincompare.sh`、`winjname.sh`）

### 11.3 両ブランチに共通

- `japanese/jlib.h` の削除（535行、どこからも include されないデッドコード）

### 11.4 訂正 ―― `sdl_queue_text()` に overread はない

Phase 0 のコミットメッセージと `win/tty/sdlterm.c` のコメントに、旧
`sdl_queue_text()` が「継続バイトか確認する前に `p[1]` と `p[2]` を読んでいた」と
書いたが、**これは誤り**である。`&&` は短絡評価するので `p[1]` が継続バイトでない
限り `p[2]` は読まれず、`p[1]` 自体は終端 NUL を読むだけで境界内に収まる。

書き換えそのものは正当で、理由は「4バイト列を無音で捨てていた」——`&&` の連鎖に
4バイトの分岐がなく、`else` が黙って読み飛ばしていた——の1点である。
