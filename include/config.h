 ...
 */
/*
 * Enabled: the Windows zip ships one nhdat instead of a hundred-odd loose
 * files, which is what every released JNetHack has done.  Note that
 * dlb_fopen() gives up entirely when nhdat is missing -- it does not fall
 * back to loose files -- so nhdat has to be in HACKDIR.  See
 * sys/unix/Makefile.dat and test/mkplaydir.sh.
 */
