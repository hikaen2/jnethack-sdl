...
if [ "$target" = sdl ]; then
    ...
    NETHACK_SEED=$seed HACKDIR="$d" NETHACKOPTIONS=color \
        python3 test/xdrive.py --keys 'n V y SPACE SPACE' --close \
    ...
    NETHACK_SEED=$seed NETHACKOPTIONS=color WINEDEBUG=${WINEDEBUG:--all} \
        NETHACK_SDL_FONT=${NETHACK_SDL_FONT:-'Z:\usr\share\fonts\opentype\noto\NotoSansCJK-Regular.ttc:5'} \
        python3 test/xdrive.py --keys 'n V y SPACE SPACE' --close \
            --cwd "$d" -- wine ./jnethack.exe -u poc
fi

# MICRO writes <plname>.sav into HACKDIR (src/files.c set_savefile_name);
# Unix writes save/<uid><plname>.
saved=$(find "$d" -maxdepth 2 \( -name '*.sav' -o -path '*/save/*' \) \
