...
# in initoptions() instead -- a showsyms difference, not a charset one,
...
NETHACK_SEED=$seed HACKDIR="$work/tty" NETHACKOPTIONS=color,DECgraphics \
    python3 test/ptydrive.py --keys "$keys" \
	--dump "$work/tty.txt" -- src/nethack.tty -u poc >/dev/null 2>&1 || true
