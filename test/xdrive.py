...
def find_window(dpy, pid_title, pid=None, cwd=None):
    ...
    def title(win):
        """The window's title, from either of the two places it can live.

        get_wm_name() reads WM_NAME, the ICCCM property.  SDL2 also sets
        _NET_WM_NAME (EWMH, UTF8_STRING), and on some versions that is the
        only one that ends up non-empty -- SDL_CreateWindow's title then
        appears nowhere in WM_NAME and a WM_NAME-only search finds nothing.
        """
        try:
